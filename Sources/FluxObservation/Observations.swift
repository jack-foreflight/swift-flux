...
public struct Observations<Element: Sendable, Failure: Error>: AsyncSequence, Sendable {
    ...
    struct State {
        ...
        static func willChange(isolation iterationIsolation: isolated (any Actor)? = #isolation, state: _ManagedCriticalState<State>, id: Int) async {
            await withUnsafeContinuation(isolation: iterationIsolation) { continuation in
                state.withCriticalRegion { state in
    ...
    ///     - isolation:  The concurrency isolation domain of the caller.
    ///     - emit: A closure to generate an element for the sequence.
    public static func untilFinished(@_inheritActorContext _ emit: @escaping @isolated(any) @Sendable () throws(Failure) -> Iteration) -> Observations<Element, Failure> {
    ...
    public struct Iterator: AsyncIteratorProtocol {
        ...
        // Pre-iOS 18.0 iteration will opt to use the enclosing #isolation to perform the `next()` operation
        public mutating func next() async throws -> Self.Element? {
            if #available(iOS 18.0, *) {
                ...
                do {
                    ...
                    if !started {
                        started = true
                        return try await trackEmission(isolation: #isolation, state: state, id: id)
                    } else {
                        ...
                        await withIsolatedTaskCancellationHandler(operation: {
                            await State.willChange(isolation: #isolation, state: state, id: id)
                        }, onCancel: {
