...
/// the ``FluxObservation/Observable()`` macro to indicate observability of a type.
public struct ObservationRegistrar: Sendable {
    ...
    private struct State: @unchecked Sendable {
        ...
        internal mutating func deinitialize() -> [(AnyKeyPath, @Sendable (AnyKeyPath) -> Void)] {
            var trackers = [(AnyKeyPath, @Sendable (AnyKeyPath) -> Void)]()
            for (keyPath, ids) in lookups {
                for id in ids {
                    if let tracker = observations[id]?.willSetTracker {
                        trackers.append((keyPath, {
                            tracker($0)
                        }))
                    }
                }
    ...
    internal struct Context: Sendable {
        ...
        internal func deinitialize() {
            ...
            for (keyPath, action) in tracking {
                action(keyPath)
            }
        }
    ...
    private final class Extent: Sendable {
        let context = Context()
    ...
    /// ``FluxObservation/ObservationRegistrar`` when using the
    /// ``FluxObservation/Observable()`` macro to indicate observably
    /// of a type.
    public init() {
    }
